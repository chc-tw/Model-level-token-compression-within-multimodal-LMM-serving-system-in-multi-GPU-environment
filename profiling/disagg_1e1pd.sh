...
CUDA_VISIBLE_DEVICES="$GPU_E" vllm serve "$MODEL" \
    ...
    --max-num-seqs 128 \
    --allowed-local-media-path ${GIT_ROOT} \
    --ec-transfer-config '{
...
python "${GIT_ROOT}/vllm/examples/online_serving/disaggregated_encoder_dynamic_sizing/disagg_epd_proxy.py" \
    --host "0.0.0.0" \
    --port "$PROXY_PORT" \
...
curl http://127.0.0.1:${PROXY_PORT}/v1/chat/completions \
    ...
    {"role": "user", "content": [
        {"type": "image_url", "image_url": {"url": "file://'"${GIT_ROOT}"'/vllm/tests/v1/ec_connector/integration/hato.jpg"}},
